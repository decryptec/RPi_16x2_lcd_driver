...
#define MAX_BUFFER_SIZE 33 // 32 char + 1 null char
// Control pins
#define reg_select 0
#define enable 5
...
static long int my_ioctl(struct file *filep, unsigned cmd, unsigned long arg)
{
    ...
    {
    case INIT:
        pr_info("lcd_driver - ioctl INIT %lu\n", arg);
        break;
    case CLEAR:
        lcd_byte(LCD_CLEAR_DISPLAY, 0); // Clear display
        pr_info("lcd_driver - ioctl clear\n");
        break;
    case LINE_1:
        lcd_byte(LCD_LINE_1,0);
        msleep(1000);
        pr_info("lcd_driver - cursor to Line 1\n");
        break;
    case LINE_2:
        lcd_byte(LCD_LINE_2,0); //Start at Line 1
        msleep(1000);
        pr_info("lcd_driver - cursor to Line 2\n");
        break;
    case scroll_left:
        ...
        pr_info("lcd_driver - ioctl scroll left by %d\n", user_input);
    ...
    if (copy_from_user(&user_input, (int32_t __user *)arg, sizeof(user_input)))
        ...
        pr_info("lcd_driver - ioctl scroll right by %d\n", user_input);
        for (int i = 0; i < user_input; ++i)
        {
...
static int __init lcd_driver_init(void)
{
    ...
    lcd_byte(LCD_CLEAR_DISPLAY, 0);  // Clear display
    lcd_byte(LCD_LINE_1,0); //Start at Line 1
    msleep(2);
